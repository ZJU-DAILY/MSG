...
void set_dist_func(hnswlib::DISTFUNC<float> distfunc) {
    fstdistfunc_ = distfunc;
}
...
float dist_comp(const float& dis, const void *data, const void *query, 
    ...
    while(i < D){
        ...
        check = std::min(delta_d, D-i);
        res += w * fstdistfunc_(q, d, &check);
        q += check;
        d += check;
        i += check;
